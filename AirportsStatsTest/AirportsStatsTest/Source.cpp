...
public:
    ...
    void AssignAirports(Items& obj) const {
        //airportsStatsVector.resize(quantity);
        for (size_t i = 0; i != obj.size(); i++) {
            //airportsStats[static_cast<TAirport>(i)] = 0;
            obj[i].first = static_cast<TAirport>(i);
            obj[i].second = arr[i];
        }
    }
    ...
    AirportCounter(TIterator begin, TIterator end) {
        ...
        for (auto it = begin; it != end; it++) {
            /*airportsStats[*it]++;*/
            arr[static_cast<uint32_t>(*it)]++;
        }
    }

    size_t Get(TAirport airport) const {
        return arr[static_cast<uint32_t>(airport)];
        //return airportsStats.at(airport);
    }

    void Insert(TAirport airport) {
        arr[static_cast<uint32_t>(airport)]++;
        /*airportsStats[airport]++;*/
    }

    void EraseOne(TAirport airport) {
        //auto ref = airportsStats[airport];
        //ref = (ref == 0) ? 0 : ref - 1; ??? 
        //airportsStats[airport]--;
        arr[static_cast<uint32_t>(airport)]--;
    }
