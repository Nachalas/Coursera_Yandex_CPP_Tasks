...
public:
    ...
    explicit ConcurrentMap(size_t bucket_count) : bucketCount(move(bucket_count)) {
        concurrentMaps.resize(bucketCount);
        mutexes = vector<mutex>(bucketCount);
    }

    WriteAccess operator[](const K& key) {
        size_t index = hasher(key) % bucketCount;
        return WriteAccess{ lock_guard<mutex>(mutexes[index]), concurrentMaps[index][key] };
    }

    ReadAccess At(const K& key) const {
        size_t index = hasher(key) % bucketCount;
        return ReadAccess{lock_guard<mutex>(mutexes[index]), concurrentMaps[index].at(key)};
    }

    bool Has(const K& key) const {
        size_t index = hasher(key) % bucketCount;
        lock_guard<mutex> g(mutexes[index]);
        if (concurrentMaps[index].find(key) != concurrentMaps[index].end()) {
            return true;
        } else{
            return false;
        }
    ...
    MapType BuildOrdinaryMap() const {
        ...
        for (size_t i = 0; i < bucketCount; i++) {
            ...
            for (const auto& j : concurrentMaps[i]) {
                toReturn[j.first] = j.second;
            }
        }
        return toReturn;
    }
